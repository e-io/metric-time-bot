...
def current_metric_time():
    ...
    answer = f'Метрическое время (по Москве, GMT+3): {millidays} миллидней и {microdays} микродней\n' \
             f'или просто {mtime}\n' \
             f'В совмещенных терминах: {deciday}:{milliday}:{centimilliday}\n' \
             f'{deciday} метрических часов {milliday}, метрических минут {centimilliday}, метрических секунд\n' \
             f'Вавилонское время: {time.hour}:{time.minute}:{time.second}'
